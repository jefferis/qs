...
if(.Platform$OS.type == "windows") {
  write_pipe <- "cat > N:/win_test.gz"
  read_pipe <- "cat N:/win_test.gz"
} else {
  write_pipe <- "brotli -0 -c > /tmp/test.br"
  read_pipe <- "brotli -dc /tmp/test.br"
}

################################################################################################

qsave_rand <- function(x, file) {
  qsave_pipe(x, write_pipe, shuffle_control = sample(0:15,1), check_hash = sample(c(T,F),1) )
}

qread_rand <- function(file) {
  x <- qread_pipe(read_pipe, use_alt_rep = sample(c(T,F),1), strict=T)
  return(x)
}
