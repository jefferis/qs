...
if(.Platform$OS.type == "windows") {
  write_pipe <- function() pipe("cat > N:/win_test.gz", "wb")
  read_pipe <- function() pipe("cat N:/win_test.gz", "rb")
} else {
  write_pipe <- function() pipe("brotli -0 -c > /tmp/test.br", "wb")
  read_pipe <- function() pipe("brotli -dc /tmp/test.br", "rb")
}
...
qsave_rand <- function(x, file) {
  p <- write_pipe()
  qsave_pipe(x, p, shuffle_control = sample(0:15,1), check_hash = sample(c(T,F),1) )
  close(p)
}

qread_rand <- function(file) {
  p <- read_pipe()
  x <- qread_pipe(p, use_alt_rep = sample(c(T,F),1), strict=T)
  close(p)
  return(x)
}
